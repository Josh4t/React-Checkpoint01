...
import { AppContext } from "../../../../server-backend/Contexts/AppContext";
...
const LogIn =() => {
  ...
  return (
    <div className='bg-slate-200 min-h-screen flex flex-col items-center justify-center px-1 md:px-10'>
        <form className='bg-white p-5 rounded-md shadow-sm w-full text-center md:w-1/2'>
        <h1 className='font-Monserrat text-3xl font-bold text-primary text-center'>
        Login
        </h1>
        <div className='w-full flex justify-center'>
            <p className='text-sm mt-3 w-4/5'>
              Welcome to <span className='oj font-bold font-iceberg'>Ooja</span>
              . Here you will get your proper market experience. please enter your login details and enjoy the experience.
            </p>
        </div>
        <div className='flex flex-col sm:flex-row justify-between m-2 sm:items-end'>
        <label htmlFor='email'
        className='font-Monserrat text-sm sm:text-xl text-primary mr-1'>
        Email:
        </label>
        <input
        ...
        disabled={loading}
        type='submit'
        className='{w-full bg-primary text-white p-2 rounded-md font-Monserrat text-xl mt-5 duration-200 ease-linear hover:bg-secondary ${loading&&"cursor-not-allowed}}'>
        {loading ? "loading..." : "login"}
        </button>
        <p className='text-sm mt-3'>
            No account?{""}
            <Link
            className='text-tertiary underline' to="/signup">
            Sign Up
            </Link>{""}
